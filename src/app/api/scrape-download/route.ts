...
import { scrapeDownloadPage } from '@/lib/scraper';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await scrapeDownloadPage(targetUrl);
    return NextResponse.json({
      success: true,
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'An error occurred during scraping the download page'
      },
      { status: 500 }
    );
  }
